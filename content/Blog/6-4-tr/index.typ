...
$
L approx sum_x log P(z^*) = sum_x sum_(v in z^*) log P(v)
$

从按句子遍历变成按 subword 遍历，对每个 subword $v$，它在整个语料所有句子的最优切法里出现了 $E[v]$ 次：

$
L approx sum_v E[v] dot log P(v)
$
]

#tufted.margin-note[删掉 $v$ 之后损失多少？

在这个近似下，$v$ 对 $L$ 的贡献是 $E[v] dot log P(v)$，所以删掉 $v$ 造成的损失近似为：

$
Delta L_v approx E[v] dot (-log P(v))
$
