...
#tufted.margin-note[*第二步*：用 EM 学概率:我们希望模型对语料的解释能力越强越好，也就是最大化语料的 log likelihood：,我们定义：$$
L = \sum{\text{Sentence} x} \log P(x)
$$]
...
$$
P(x) = \sum{z}P(z)
$$。切法是一个隐变量，我们不知道一个句子“正确”的切法是什么。所以对所有可能的切法求和，把隐变量边缘化掉，这是概率论处理中隐变量的标准做法。
同时Unigram LM 假设每个 subword 是独立生成的，所以一种切法的概率就是各 subword 概率的乘积。这也是模型名字“Unigram”的由来。]
...
$$
E[v] = \sum_x \sum_z \frac{P(z)}{P(x)} \cdot \text{count}(v, z)
$$

直观来说：概率高的切法权重大，$v$ 在这些切法里出现得越多，期望计数就越高。]

#tufted.margin-note[M-step：更新概率

用期望计数归一化，更新每个 subword 的概率：

$$
P(v) = \frac{E[v]}{\sum{v'} E[v']}
$$

E-step 和 M-step 交替进行，直到概率收敛。]

#tufted.margin-note[*第三步*：计算每个 subword 的重要性：EM 收敛后，我们得到了每个 subword 的 $P(v)$ 和 $E[v]$。

现在要决定删哪些 subword。直觉是：删掉某个 subword 后，语料的 log likelihood 掉得越少，说明它越不重要。]

#tufted.margin-note[EM 收敛后，对每个句子，概率最高的那种切法会占主导：

$$
P(x) = \sum_z P(z) \approx P(z^\*)
$$

所以 log likelihood 近似为：

$$
L \approx \sum_x \log P(z^\*) = \sum_x \sum{v \in z^\*} \log P(v)
$$

换一个角度，从按句子遍历变成按 subword 遍历。对每个 subword $v$，它在整个语料所有句子的最优切法里出现了 $E[v]$ 次，所以：

$$
L \approx \sum_v E[v] \cdot \log P(v)
$$
]

#tufted.margin-note[删掉 $v$ 之后损失多少？

在这个近似下，$v$ 对 $L$ 的贡献是 $E[v] \cdot \log P(v)$，所以删掉 $v$ 造成的损失近似为：

$$
\Delta L_v \approx E[v] \cdot (-\log P(v))
$$

其中：

- $E[v]$：$v$ 被用了多少次，次数越多删掉影响越大
- $-\log P(v)$：$v$ 每次被用时的贡献大小
