...
#table(
  ...
  table.header([*局限*], [*具体表现*], [*为什么深度学习能解决*]),
  [*特征工程*], [模型本身不处理原始数据。图像必须先人工设计SIFT/HOG特征，文本必须先人工做TF-IDF/Bag-of-Words。这需要领域专家花数月。], [CNN/Transformer *直接从原始像素/字符学习特征*，不需要人工设计], [*高维结构化数据*], [图像有空间结构，文本有序列结构，语音有时间结构。线性模型和树模型把它们展平后，*空间/序列关系全部丢失*。], [CNN 保留局部空间关系，RNN/Transformer 保留序列依赖], [*数据规模天花板*], [SVM 在百万级数据上训练很慢；树模型在特征数爆炸时（如图像每个像素一个特征）根本没法工作。], [神经网络 + GPU 并行，可以处理十亿级数据], [*非线性逼近能力*], [单层/浅层模型对复杂函数（如"图像中有猫"）的逼近能力有限。], [深层网络（ResNet、Transformer）可以逼近任意复杂函数], [*多模态融合*], [如何把图像特征向量 + 文本特征向量 + 音频特征向量融合？传统方法靠拼接后接SVM，很粗糙。], [多模态大模型（CLIP、GPT-4o）在*统一空间*中对齐所有模态]
)
...
*小幽默*：ReLU 就像一个绝对理性的打工人：给正反馈（$z > 0$）就全情投入，给负反馈（$z < 0$）就直接躺平输出0。虽然有点玻璃心（会死），但效率是真的高。深度学习后来能烧得起千亿参数，ReLU 的简洁功不可没。
