...
#table(
  columns: (1fr, 1fr),
  align: (left, left),
  table.header([*局限*], [*具体表现*]),
  [*特征工程*], [模型本身不处理原始数据。图像必须先人工设计SIFT/HOG特征，文本必须先人工做TF-IDF/Bag-of-Words。这需要领域专家花数月。], [*高维结构化数据*], [图像有空间结构，文本有序列结构，语音有时间结构。线性模型和树模型把它们展平后，*空间/序列关系全部丢失*。], [*数据规模天花板*], [SVM 在百万级数据上训练很慢；树模型在特征数爆炸时（如图像每个像素一个特征）根本没法工作。], [*非线性逼近能力*], [单层/浅层模型对复杂函数（如"图像中有猫"）的逼近能力有限。],  [*多模态融合*], [如何把图像特征向量 + 文本特征向量 + 音频特征向量融合？传统方法靠拼接后接SVM，很粗糙。]
)
