...
> 你可能会第一次接触到终端和命令行，不要害怕，它们只是和计算机交流的一种方式。
...
完成后打开**终端**，输入并运行 `typst --version`，如果显示版本号则表示安装成功。

### 2. 安装 uv

_如果你的系统已经安装 Python，也可以跳过这一步不安装 uv。_
...
- 拥有一个 GitHub 账号，从而使用 GitHub Pages 进行免费部署
...
这会创建 `D:\Yousa-Mirage.github.io.git\` 路径，并将项目文件下载到该目录下。接下来你可以重命名该文件夹为你喜欢的名字，例如 `D:\My-Website\`。这就是我们以后的本地网站项目目录，我们在其中编辑文档、运行构建脚本。
...
如果你安装了 `make`，也可以运行以下命令：

```bash
make build
```

此命令会将 `content/` 下的 `.typ` 文件对应编译为 HTML，并输出到 `_site/` 目录。`_site/` 目录就是你的网站在本地的样子。在你完成修改后，每次运行该命令即可重新构建网站。
...
默认 `content/` 包含进一步的文档说明和示例页面，你可以自行探索和修改。推荐在启动本地预览后，一边阅读网页一边对照生成该网页的 `.typ` 源代码，从而更好地了解 Typst 文本内容和如何编写你自己的网站。

在了解基本网页结构和如何编写后，你需要将 `content/` 中的内容替换为你自己的内容，从而完成你的网站。
